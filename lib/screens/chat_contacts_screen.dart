...
class _ChatContactsScreenState extends State<ChatContactsScreen> {
  ...
  // Firestore whereIn limit is 10 — chunk ids to fetch users efficiently
  ...
  Future<List<_ChatContact>> _buildContacts(
    ...
    // Preload chat docs to read lastMessage/updatedAt/unread state
    ...
    final contacts = users.entries.map((entry) {
      ...
      final chatData = chatDoc != null ? chatDoc.data() as Map<String, dynamic>? : null;
    ...
    // Sort: pinned first, then by last update desc, then by name
    contacts.sort((a, b) {
      if (a.isPinned != b.isPinned) return a.isPinned ? -1 : 1;
      final aTime = a.lastMessageAt?.toDate() ?? DateTime.fromMillisecondsSinceEpoch(0);
      final bTime = b.lastMessageAt?.toDate() ?? DateTime.fromMillisecondsSinceEpoch(0);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      backgroundColor: Colors.white,
      body: SafeArea(
        top: false, // tránh chừa thêm khoảng trắng phía trên vì đã có AppBar
        child: StreamBuilder<DocumentSnapshot>(
          ...
          builder: (context, userSnapshot) {
            ...
            final pinnedList = List<String>.from(userData['pinnedChatContacts'] ?? const []);
            ...
            return StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return const Center(child: Text('Bạn chưa theo dõi ai để trò chuyện.'));
                }


                final followingIds = snapshot.data!.docs
                    .map((doc) => (doc.data() as Map<String, dynamic>)['followingId'])
                    .whereType<String>()
                    .toSet()
                    .toList();
                ...
                if (followingIds.isEmpty) {
                  return const Center(child: Text('Chưa có người nào trong danh sách trò chuyện.'));
                }
                      ...
                      children: [
                        ...
                        Expanded(
                          child: filtered.isEmpty
                              ? const Center(child: Text('Không tìm thấy danh bạ hợp lệ.'))
                              : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                trailing: IconButton(
                                  ...
                                  onPressed: () => _togglePin(contact.userId, contact.isPinned),
                                ),
                                onTap: () {
                                  Navigator.push(
                                    context,
